...
allHtmlFiles.forEach(file => {
    ...
    if (!content.includes('mascot.js')) {
        // Inject right before </body> to ensure all DOM is ready
        content = content.replace('</body>', `  <script src="/js/mascot.js" defer></script>\n</body>`);
        fs.writeFileSync(file, content, 'utf8');
        injectedCount++;
    }
});

console.log('✅ Injected Global Mascot & Easter Eggs into ' + injectedCount + ' HTML files.');
